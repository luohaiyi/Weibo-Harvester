...
> - 本模块已经支持 `--config <path>` 启动协议，GUI 会为每次运行生成唯一配置文件并传给 `weibo.py`。
> - 当前工作区默认输出目录收口到 `/app/data/weibo-crawler`，日志收口到 `/app/logs/weibo-crawler`。
> - 下文主体仍保留上游模块原始说明，主要用于参数语义和能力参考；若与根 README 或 GUI 文档冲突，以当前工作区文档为准。
