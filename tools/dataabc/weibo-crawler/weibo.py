...
class Weibo(object):
    ...
    def user_to_mysql(self):
        """将爬取的用户信息写入MySQL数据库"""
        mysql_config = self.mysql_config or {
            "host": "localhost",
            "port": 3306,
            "user": "root",
            "password": "123456",
        ...
        db_name = mysql_config.get('database', 'weibo_harvester')
        # 创建数据库
        create_database = f"CREATE DATABASE IF NOT EXISTS {db_name} DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"
    ...
    def parse_weibo(self, weibo_info):
        ...
        weibo["links"] = ",".join(self.get_link_urls(selector))
    ...
    def info_to_mongodb(self, collection, info_list):
        ...
        try:
            ...
            mongo_db_name = (self.mysql_config or {}).get('database', 'weibo_harvester')
    ...
    def mysql_ensure_column(self, mysql_config, table, col_name, col_def):
        """为已有表安全添加列（列已存在则跳过）"""
        import pymysql
        if self.mysql_config:
            mysql_config = self.mysql_config
        db_name = mysql_config.get('database', 'weibo_harvester') if isinstance(mysql_config, dict) else 'weibo_harvester'
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if not cursor.fetchone():
                    ...
                    connection.commit()
                    logger.info("MySQL 表 %s 已添加列 %s", table, col_name)
        finally:
            connection.close()
    ...
    def weibo_to_mysql(self, wrote_count):
        ...
        self.mysql_create_table(mysql_config, create_table)
        # 为旧表补充可能缺失的列（CREATE TABLE IF NOT EXISTS 不会修改已有表）
        self.mysql_ensure_column(mysql_config, "weibo",
                                 "live_photo_url", "varchar(1000)")
        self.mysql_ensure_column(mysql_config, "weibo",
                                 "links", "text")
        ...
        for w in info_list:
            ...
            if "retweet" in w:
                ...
                w["retweet_id"] = ""
            weibo_list.append(w)
        # 插入微博数据（dict 已在前置循环中处理好，字段与表列一致）
    ...
    def weibo_to_sqlite(self, wrote_count):
        ...
        info_list = copy.deepcopy(self.weibo[wrote_count:])
        for w in info_list:
            # 将 full_created_at 就地改名为 created_at，使 dict key 与表列名一致
            w["created_at"] = w["full_created_at"]
            del w["full_created_at"]
    ...
    def sqlite_insert_weibo(self, con: sqlite3.Connection, weibo: dict):
        """插入微博数据到 SQLite，dict key 已与表列名对齐"""
        self.sqlite_insert(con, weibo, "weibo")
    ...
    def get_sqlite_connection(self):
        ...
        # 始终执行建表（CREATE TABLE IF NOT EXISTS 是幂等的），
        # 避免数据库文件已存在但缺少表时（如旧版本残留/上次崩溃）导致 INSERT 报错
