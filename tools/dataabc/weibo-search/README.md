...
> ## Harvester 集成说明（优先阅读）
> - 当前仓库推荐入口是根目录 `docker compose up -d` 后访问 GUI，而不是在宿主机直接单跑 Scrapy 命令。
> - 本模块已经补了正式入口 `run.py --config <path>`，GUI 会为每次运行生成唯一配置文件并传入。
> - 当前工作区默认输出目录收口到 `/app/data/weibo-search/images` 与 `/app/data/weibo-search/files`，日志统一落到 `/app/logs/weibo-search`。
> - 下文主体仍保留上游模块原始说明，主要用于参数与抓取策略参考；若与根 README 或 GUI 文档冲突，以当前工作区文档为准。
