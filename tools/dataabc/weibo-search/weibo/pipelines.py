...
class MyImagesPipeline(ImagesPipeline):
    ...
    def file_path(self, request, response=None, info=None, *, item=None):
        ...
        item = item or request.meta.get('item')
        sign = request.meta.get('sign', '')
        weibo = item.get('weibo') if isinstance(item, dict) else None
        weibo_id = weibo.get('id', 'unknown') if weibo else 'unknown'
        keyword = item.get('keyword', 'unknown') if isinstance(item, dict) else 'unknown'
        base_dir = '结果文件' + os.sep + keyword + os.sep + 'images'
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
        image_suffix = image_url[image_url.rfind('.'):]
        file_path = base_dir + os.sep + weibo_id + sign + image_suffix
        return file_path
...
class MyVideoPipeline(FilesPipeline):
    ...
    def file_path(self, request, response=None, info=None, *, item=None):
        ...
        weibo_id = weibo.get('id', 'unknown') if weibo else 'unknown'
        keyword = item.get('keyword', 'unknown') if isinstance(item, dict) else 'unknown'
        base_dir = '结果文件' + os.sep + keyword + os.sep + 'videos'
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
        file_path = base_dir + os.sep + weibo_id + '.mp4'
        return file_path
...
class MongoPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            if weibo is None:
                return item
            if not self.collection.find_one({'id': weibo['id']}):
                self.collection.insert_one(dict(weibo))
            else:
                self.collection.update_one({'id': weibo['id']},
                                           {'$set': dict(weibo)})
...
class DuplicatesPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if weibo is None:
            return item
        if weibo['id'] in self.ids_seen:
            raise DropItem("过滤重复微博: %s" % item)
        else:
            self.ids_seen.add(weibo['id'])
            return item
