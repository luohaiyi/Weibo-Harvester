...
> ## Harvester 集成说明（优先阅读）
> - 当前仓库推荐入口是根目录 `docker compose up -d` 后访问 GUI，而不是在宿主机直接单跑本模块。
> - 本模块已经支持 `--config <path>` 启动协议，GUI 会为每次运行生成唯一配置文件并传给 `weibo_follow.py`。
> - GUI 默认会把输出文件写到 `/app/data/weibo-follow`，日志统一落到 `/app/logs/weibo-follow`。
> - 下文主体仍保留上游模块原始说明，主要用于参数和原始行为参考；若与根 README 或 GUI 文档冲突，以当前工作区文档为准。
